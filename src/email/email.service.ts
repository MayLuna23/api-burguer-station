...
export class EmailService {
  ...
  async sendEmail(data: {
    ...
    this.logger.log(`[EmailService][${method}] Preparing to send email to ${data.email}`);
    ...
    try {
      ...
      this.logger.log(`[EmailService][${method}] Email sent successfully to ${data.email}`);
      this.logger.debug(`[EmailService][${method}] SendGrid response: ${JSON.stringify(response)}`);
      return {
        statusCode: 200,
      ...
      this.logger.error(
        `[EmailService][${method}] Error sending email: ${JSON.stringify(errorBody)}`,
        error.stack,
      );
  ...
  private generateEmailHtml(data: { email: string; total: number; products: ProductsSold[] }): string {
    return `
        ...
        ${data.products
          .map(
            ...
            <div style="border-bottom:1px solid #ccc; padding:16px 0;">
              ...
              <p style="font-size: 24px; margin:4px 0; text-align:right; font-weight:bold; color:#f97316;">
